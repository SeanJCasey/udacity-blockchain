...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(isOwner(msg.sender), "Account is not the contract owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), "Owner account not valid");
        _owner = newOwner;
        emit OwnershipTransfered(_owner);
    }

    function isOwner(address _address) view internal returns (bool) {
...
contract Pausable is Ownable {
    ...
    event Paused(address _triggered);

    event Unpaused(address _triggered);
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "Transfer address cannot be token owner");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(isOwner(msg.sender) || isApprovedForAll(ownerOf(tokenId), msg.sender));
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "_transferFrom: must be from token owner");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "_transferFrom: must not be to empty address");
